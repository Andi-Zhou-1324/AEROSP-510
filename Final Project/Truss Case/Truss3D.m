...
%external forces
force_node = [9];
F = assignForce(F,force_node, repmat([100,0,0],size(force_node,1),1),dof);
...
function plot_edge(nodal_list, connectivity_matrix,color,labelSwitch)
    ...
    for i = 1:size(connectivity_matrix, 1)
        ...
        line([start_node(1), end_node(1)], [start_node(2), end_node(2)], 'Color', color);
    end
