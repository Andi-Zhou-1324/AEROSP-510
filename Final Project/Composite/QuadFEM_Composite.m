...
fun = @(fiberAngle)compositeMinimizationStudy(fiberAngle);

[fiberAngle_optimal, fval] = fminunc(fun,fiberAngle_0)




%% Functions Declared
function [stress_von_mise] = compositeMinimizationStudy(fiberAngle)
    nelem = 30
    [max_stress] = FEMquad(nelem,100,fiberAngle);
    stress_von_mise = sqrt(max_stress(1).^2 - (max_stress(1).*max_stress(2)) + max_stress(2).^2 + (3.*max_stress(3).^2));
end


function compositeAngleStudy(fiberAngle)
    stress_max = [];
    nelem = 50;
    for i = 1:size(fiberAngle,1)
        [max_stress] = FEMquad(nelem,100,fiberAngle(i,:));
        stress_max = [stress_max,max_stress];
    end
    figure()
    plot(fiberAngle(:,1),stress_max);
...
function compositeConvergenceStudy(fiberAngle) %function to do convergence study
        stress_max = [];
        for nelem = 2:2:50
            [max_stress] = FEMquad(nelem,50,fiberAngle);
            stress_max = [stress_max,max_stress];
        end
        figure()
...
function [max_stress] = FEMquad(nelem,ENDELEM,fiberAngle)
    ...
    for A = 1:Nel
        ...
        localstiffness = localstiffnessmat(intpts,coord,fiberAngle); %HW6, p3
    ...
    if nelem == ENDELEM
        ...
        patch('Faces',e,'Vertices',co + reshape(u,2,[])','FaceColor','none','EdgeColor','r')
        hold off
        title('Resulting Deformation')
        xlabel('x')
        ylabel('y(m)')
        xlabel('x(m)')
    end
...
function k = localstiffnessmat(intpts,coords,fiberAngle) %hw6 p4
    k = zeros(8,8);
    t = 0.01;
    E = 70e9;nu = 0.3;
    D = calculateD(fiberAngle);
